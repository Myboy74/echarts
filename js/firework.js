    ...
    $.fn.fireworks = function(options){
        ...
        function launch() {
            if (rockets.length < 10) {
                ...
                rocket.v.x = Math.random() * 24 - 3;
                rocket.v.y = Math.random() * -30 -4;
                rocket.color = Math.floor(Math.random() * 720 / 10) * 10;
                rockets.push(rocket);
            }
        }
        function loop () {
            ...
            context.fillStyle = "rgba(10,13,32,0)";
        ...
        Particle.prototype.render = function () {
            ...
            gradient.addColorStop(0.9, "hsla(" + this.color + ",100%, 50%, 0.5)");
            gradient.addColorStop(1, "rgba(10, 12, 13, 1)");
        ...
        Rocket.prototype.render = function (context) {
            var x = this.pos.x,
                y = this.pos.y,
                r = this.size/2;
            var gradient = context.createRadialGradient(x, y, 0.5, x, y, r);
            gradient.addColorStop(0, "#1c2f49");
            gradient.addColorStop(1, "rgba(10, 12, 33, 0.5)");
            context.fillStyle = gradient;
            context.beginPath();
            context.arc(x, y, r, 0, 2 * Math.PI, true);
            context.closePath();
        ...
        Rocket.prototype.explode = function () {
            var count = getGaussianDistributionNumber(1000, 20);
            for (var i = 0; i <= count; i++) {
                var particle = new Particle(this.pos);
                var angle = Math.random() * Math.PI * 5;
                var speed =  getGaussianDistributionNumber(15,1);
                particle.size = 3;
